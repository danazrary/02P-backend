// backend/routes/seller/sellerAiProductRoutes.js
...
import validatePublicProductUrl, {
  ProductUrlValidationError,
} from "../utils/validatePublicProductUrl.js";
import validateSupportedMarketplaceUrl, {
  UnsupportedMarketplaceError,
} from "../utils/validateSupportedMarketplaceUrl.js";
import extractProductPageBasic, {
  ProductExtractionError,
  evaluateExtractionQuality,
} from "../utils/extractProductPageBasic.js";
import {
...
function aiDebugEnabled() {
  return (
    String(process.env.AI_IMPORT_DEBUG || "").toLowerCase() === "true" ||
...
function estimateCost(tokens, envName) {
  ...
  if (!Number.isFinite(perMillion) || !Number.isFinite(Number(tokens)))
    return null;
...
function geminiUsage(response) {
  ...
  const input =
    Number(usage.promptTokenCount ?? usage.inputTokenCount ?? 0) || null;
  const output =
    Number(usage.candidatesTokenCount ?? usage.outputTokenCount ?? 0) || null;
  const total =
    Number(usage.totalTokenCount ?? (input || 0) + (output || 0)) || null;
  return {
    input_tokens: input,
    output_tokens: output,
    total_tokens: total,
    estimated_input_cost_usd: estimateCost(
      input,
      "GEMINI_INPUT_COST_PER_MILLION_USD",
    ),
    estimated_output_cost_usd: estimateCost(
      output,
...
function safeGeminiErrorDetails(error) {
  ...
  const status = Number.isFinite(numericStatus)
    ? numericStatus
    : rawStatus || null;
...
function normalizeResult(value) {
  const result = {
    ...
    features: list(value?.features, 15)
      .map((x) => ({
        text_ku: text(x?.text_ku, 100),
        text_ar: text(x?.text_ar, 100),
        value_ku: text(x?.value_ku, 300),
        value_ar: text(x?.value_ar, 300),
      }))
      .filter((x) => x.text_ku || x.text_ar || x.value_ku || x.value_ar),
    options: list(value?.options, 5)
      .map((x) => ({
        type_ku: text(x?.type_ku, 80),
        type_ar: text(x?.type_ar, 80),
        values: list(x?.values, 5)
          .map((v) => ({ ku: text(v?.ku, 100), ar: text(v?.ar, 100) }))
          .filter((v) => v.ku || v.ar),
      }))
      .filter((x) => (x.type_ku || x.type_ar) && x.values.length),
    customFieldsKu: list(value?.customFieldsKu, 15)
      .map((x) => ({ name: text(x?.name, 100), value: text(x?.value, 300) }))
      .filter((x) => x.name || x.value),
    customFieldsAr: list(value?.customFieldsAr, 15)
      .map((x) => ({ name: text(x?.name, 100), value: text(x?.value, 300) }))
      .filter((x) => x.name || x.value),
    ...
    price:
      Number.isFinite(Number(value?.price)) && Number(value.price) >= 0
        ? Number(value.price)
        : null,
    currency: ["USD", "IQD"].includes(String(value?.currency).toUpperCase())
      ? String(value.currency).toUpperCase()
      : "USD",
    imageUrls: list(value?.imageUrls, 8)
  ...
  if (
    ...
    const error = new Error(
      "AI output did not contain usable product content.",
    );
...
function failImport(res, status, code, message, state) {
  ...
  return fail(res, status, code, message, {
    requestId: state.requestId,
    ...(aiDebugEnabled() ? { debug: debugObject(state) } : {}),
  });
}

async function logUsage(values) {
  try {
    await SellerAiUsage.create(values);
  } catch (error) {
    console.error("AI usage log failed:", error?.message || error);
  }
}

async function debitCreditAndLog({ sellerId, usage }) {
  return sequelize.transaction(async (transaction) => {
    const balance = await SellerAiBalance.findOne({
    ...
    await SellerAiUsage.create(
      {
        ...usage,
        success: true,
        status: "success",
        failure_stage: "success",
        seller_credit_consumed: true,
      },
      { transaction },
    );
    return balance;
...
// 1) GET /ai-credits (کراوەیە بۆ بەکارهێنەرانی چوونەژوورەوەبوو)
router.get(
  "/ai-credits",
  ...
  async (req, res) => {
    try {
      ...
      const [balance, setting] = await Promise.all([
        getOrCreateSellerAiBalance(sellerId),
        getAiFeatureSetting(),
      ]);
      ...
      return res.json({
        ...
        data: {
          ...
          totalFreeCredits: balance.total_free_credits,
          totalPurchased: balance.total_purchased,
          totalUsed: balance.total_used,
          feature: serializeAiFeatureStatus(setting),
        },
      });
    } catch (error) {
...
// 2) GET /ai-credit-plans (کراوەیە بۆ بەکارهێنەرانی چوونەژوورەوەبوو)
router.get(
  "/ai-credit-plans",
  jwtVerifySellerToken,
  attachActor,
  async (_req, res) => {
    try {
      ...
      return fail(
        ...
        500,
        "AI_FAILED",
        "AI credit plans could not be loaded.",
      );
    }
  },
);

// 3) POST /ai-credit-purchase-request
router.post(
  "/ai-credit-purchase-request",
  purchaseLimiter,
  jwtVerifySellerToken,
  attachActor,
  canManageSettings,
  async (req, res) => {
    try {
      const sellerId =
        res.locals.sellerId || req.user?.seller_id || req.user?.id;
      const result = await createAiCreditPurchaseRequest({
        sellerId,
      ...
      return res.json({
        success: true,
        data: {
          requestId: result.request.request_code,
      ...
      return fail(
        ...
        status,
        code,
        error?.message || "AI credit purchase request could not be created.",
      );
    }
  },
);

// 4) POST /products/ai-import
router.post(
  "/products/ai-import",
  importLimiter,
  jwtVerifySellerToken,
  ...
  async (req, res) => {
    ...
    let usageBase = {
      ...
      source_url: state.sourceUrl || "unknown",
      success: false,
      status: "failed",
      gemini_request_sent: false,
      seller_credit_consumed: false,
    ...
    try {
      ...
      try {
        ...
        throw new ProductUrlValidationError(
          "INVALID_URL",
      ...
      if (balance.credit_balance <= 0) {
        await logUsage({
          ...usageBase,
          failure_stage: "credit_check",
          failure_code: "AI_CREDITS_EMPTY",
      ...
      Object.assign(state, extracted.diagnostics || {}, {
        finalHostname: hostnameOf(extracted.sourceUrl || validated.href),
      });
      logStage(state, { stage: "extraction" });
      ...
      logStage(state, {
        ...
        blockedPageDetected: quality.blockedPageDetected,
        blockedPageReason: quality.blockedPageReason,
      });
      if (!quality.passed) {
        const code =
          quality.reason === "blocked_page_detected"
            ? "WEBSITE_BLOCKED_OR_UNREADABLE"
            : "INSUFFICIENT_PRODUCT_DATA";
        const message =
          code === "WEBSITE_BLOCKED_OR_UNREADABLE"
            ? `${marketplace === "aliexpress" ? "AliExpress" : "This marketplace"} did not provide a readable product page.`
            : "The product page did not provide enough usable information.";
        await logUsage({
          ...usageBase,
        ...
        return failImport(
          res,
          code === "WEBSITE_BLOCKED_OR_UNREADABLE" ? 422 : 422,
          code,
          message,
          state,
        );
      ...
      if (!process.env.GEMINI_API_KEY) {
        ...
        await logUsage({
          ...usageBase,
          failure_stage: "system_health",
          failure_code: "AI_IMPORT_TEMPORARILY_DISABLED",
          duration_ms: Date.now() - state.startedAt,
        });
        return failImport(
          res,
          503,
          "AI_IMPORT_TEMPORARILY_DISABLED",
          safeDisabledMessage(),
          state,
        );
      }
      ...
      try {
        ...
        await recordAiGeminiFailure(
          `${failureCode}: ${geminiError.providerStatus || ""} ${geminiError.providerMessage}`.slice(
            0,
            500,
          ),
        );
        await logUsage({
          ...usageBase,
          ...usageTokens,
          gemini_request_sent: true,
          seller_credit_consumed: false,
          failure_stage: "gemini_request",
        ...
        return fail(
          ...
          502,
          failureCode,
          "AI could not prepare this product. Please try again.",
          {
            requestId: state.requestId,
            ...(aiDebugEnabled()
              ? {
      ...
      try {
        ...
        return failImport(
          ...
          502,
          "AI_FAILED",
          "AI returned incomplete product data. Please try again.",
          state,
        );
      }

      try {
        const updatedBalance = await debitCreditAndLog({
          sellerId,
          usage: {
            ...usageBase,
            ...usageTokens,
        ...
        logStage(state, {
          stage: "success",
          sellerCreditConsumed: true,
          durationMs: Date.now() - state.startedAt,
        });
        return res.json({
          success: true,
          requestId: state.requestId,
          data: { ...data, creditBalance: updatedBalance.credit_balance },
        });
      } catch (error) {
        await recordAiSystemFailure(
          error?.message || "AI import credit transaction failed",
        );
        await logUsage({
          ...usageBase,
          ...usageTokens,
          ...
          failure_code: error?.code || "DATABASE_ERROR",
          duration_ms: Date.now() - state.startedAt,
        });
        return failImport(
          res,
          500,
          "DATABASE_ERROR",
          "AI import could not be completed. No AI credit was used.",
          state,
        );
      }
    } catch (error) {
      ...
      if (error instanceof ProductExtractionError) {
        ...
        const status =
          error.code === "EXTRACTION_TIMEOUT"
            ? 504
            : error.code === "WEBSITE_BLOCKED_OR_UNREADABLE"
              ? 422
              : 502;
      ...
      if (!isGeminiServiceFailure(error))
        await recordAiSystemFailure(
          error?.message || "Unexpected AI import error",
        );
      ...
      return failImport(
        ...
        500,
        "EXTRACTION_FAILED",
        "The product page could not be imported. No AI credit was used.",
        state,
      );
    }
  },
