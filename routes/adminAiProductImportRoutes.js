...
router.post("/ai-product-import/disable", adminAuth, async (req, res) => {
  try {
    const setting = await disableAiImportManually(
      req.body?.reason || "Maintenance",
    );
    return res.json({ success: true, data: serializeAiFeatureStatus(setting) });
  } catch (error) {
...
router.get("/ai-credit-purchase-requests", adminAuth, async (req, res) => {
  try {
    ...
    return fail(
      ...
      500,
      "AI_FAILED",
      "AI credit purchase requests could not be loaded.",
    );
  }
});

router.post(
  "/ai-credit-purchase-requests/:requestId/approve",
  adminAuth,
  async (req, res) => {
    try {
      ...
      return fail(
        ...
        status,
        code,
        error?.message || "AI credit purchase request could not be approved.",
      );
    }
  },
);
...
router.post(
  "/ai-credit-purchase-requests/:requestId/reject",
  adminAuth,
  async (req, res) => {
    try {
      ...
      return fail(
        ...
        status,
        code,
        error?.message || "AI credit purchase request could not be rejected.",
      );
    }
  },
);
