...
${
  seo.type === "product"
    ? `<meta property="product:price:amount" content="${escapeHtml(seo.price)}">
...
function categorySeo(seller, categoryName, subcategoryName, requestSlug) {
  ...
  const description = truncateMetaDescription(
    `Browse ${label} products from ${shopName}.`,
  );
  const image = getAbsoluteImageUrl(seller.shop_image);
  return {
    title: `${label} | ${shopName} - Dwkan Link`,
    ogTitle: `${label} - ${shopName}`,
    description,
    canonicalUrl,
    image,
    type: "website",
    jsonLd: {
      ...
      description,
      url: canonicalUrl,
      isPartOf: `https://${seller.shop_name}.${process.env.BASE_DOMAIN || "dwkanlink.com"}/`,
    },
  };
}
  ...
  return products
    .map((product) => {
      const title = stripHtml(
        product.titleKu || product.titleAr || `Product ${product.id}`,
      );
...
async function renderShop(res, seller) {
  ...
  return sendHtml(
    ...
    injectSeoIntoHtml(
      seo,
      `<h1>${escapeHtml(seo.ogTitle)}</h1><p>${escapeHtml(seo.description)}</p>${productLinks(products, seller)}`,
    ),
  );
}
...
async function renderProduct(res, seller, productId) {
  ...
  if (!product)
    return res
      ...
      .type("html")
      .send(
        '<!doctype html><title>Product not found</title><meta name="robots" content="noindex"><h1>Product not found</h1>',
      );
  ...
  return res
    ...
    .send(
      injectSeoIntoHtml(
        seo,
        `<h1>${escapeHtml(seo.ogTitle)}</h1><img src="${escapeHtml(seo.image)}" alt="${escapeHtml(`${seo.ogTitle} - ${seller.shop_name}`)}"><p>${escapeHtml(seo.description)}</p>`,
      ),
    );
}
...
async function renderCategory(res, seller, categorySlug, subcategorySlug) {
  ...
  const subcategoryKey =
    subcategorySlug && findBySlug(category?.subcategories, subcategorySlug);
  if (!category || (subcategorySlug && !subcategoryKey))
    return res
      .status(404)
      .type("html")
      .send(
        '<!doctype html><title>Category not found</title><meta name="robots" content="noindex"><h1>Category not found</h1>',
      );
  ...
  const products = await Product.findAll({
    ...
    attributes: ["id", "titleKu", "titleAr", "descriptionKu", "descriptionAr"],
    order: [["updatedAt", "DESC"]],
    limit: 80,
  });
  const seo = categorySeo(
    ...
    categoryName,
    subcategoryName,
    [categorySlug, subcategorySlug].filter(Boolean).join("/"),
  );
  return res
    .type("html")
    .send(
      injectSeoIntoHtml(
        seo,
        `<h1>${escapeHtml(seo.ogTitle)}</h1><p>${escapeHtml(seo.description)}</p>${productLinks(products, seller, true)}`,
      ),
    );
}
  ...
  try {
    if (isMainDomain(host)) {
      return sendHtml(
        res,
        200,
        injectSeoIntoHtml(mainSiteSeo(), "<h1>Dwkan Link</h1>"),
      );
    }
    ...
    if (!isValidShopSubdomain(host)) {
      return sendHtml(
        res,
        404,
        errorHtml("Shop Not Found", "The requested shop does not exist."),
      );
    }
    ...
    return sendHtml(
      ...
      errorHtml(
        "Page Unavailable",
        "The shop page is temporarily unavailable.",
      ),
    );
  }
}
...
export default async function seoPrerender(req, res, next) {
  if (req.method !== "GET" || !BOT_AGENTS.test(req.get("user-agent") || ""))
    return next();
  ...
  try {
    ...
    if (route.length === 0 || (route.length === 1 && route[0] === "profile"))
      return renderShop(res, seller);
    if (route.length === 2 && route[0] === "p")
      return renderProduct(res, seller, route[1]);
    if (route[0] === "c" && (route.length === 2 || route.length === 3))
      return renderCategory(res, seller, route[1], route[2]);
    return next();
  } catch (error) {
